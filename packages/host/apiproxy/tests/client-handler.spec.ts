...
function scriptedApi(overrides: {
  ...
  return {
    ...
    host: {
      ...
      listEntries: r => ok(r, { path: '/t', entries: [] }),
      searchEntries: r => ok(r, { entries: [], truncated: false }),
      mkdir: r => ok(r, { path: '/t/new' }),
      rename: r => ok(r, { path: '/t/renamed' }),
      delete: r => ok(r, { deleted: true as const }),
      copy: r => ok(r, { path: '/t/copy' }),
      writeText: r => ok(r, { path: r.payload.path }),
      readText: r => ok(r, { path: r.payload.path, content: '' }),
      openPath: r => ok(r, { opened: true as const }),
      revealPath: r => ok(r, { revealed: true as const }),
      restartWeb: r => ok(r, { accepted: true as const, port: r.payload.port ?? 3080 }),
      ...overrides.host,
    },
    workspace: {
