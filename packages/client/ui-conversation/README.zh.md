...
- **没有 Cursor 的 Ask / Agent 切换、Inline Edit（Ctrl+K）或 Tab 补全**：composer 没有编辑模式开关。Access chip 提交 `/permission <preset>`；plan mode 走 `/plan`；skill 走 `/name`；`@` 同时覆盖工作区文件／文件夹路径芯片（`workspace-file`）与正在运行的 subagent。[Web UI 指南](../../../docs/user/guide/index.md#composer-vs-cursor) 映射这些控件。
