...
The **local-filesystem implementation** of the `ctx.fs` provider contract ([`@deepseek-ai/dsh-fs`](../fs)). Backs the sixteen `FileSystem` primitives with the host filesystem; loading it as a plugin populates `ctx.fs`.
...
- **`mkdir`** — creates one directory. The parent must already exist (`FS_NOT_FOUND` otherwise). An existing target fails with `FS_ALREADY_EXISTS` rather than succeeding as a no-op.
- **`rename`** — moves a file or directory. An existing destination is `FS_ALREADY_EXISTS`; a missing source is `FS_NOT_FOUND`; the same identity on both sides is `FS_IO_ERROR`. Cross-target locking is ordered by `targetKey` to avoid deadlock.
- **`delete`** — removes a file or directory recursively. A missing target fails with `FS_NOT_FOUND` rather than succeeding as a no-op.
- **`copy`** — copies a file or directory recursively (`errorOnExist`). An existing destination is `FS_ALREADY_EXISTS`; a missing source is `FS_NOT_FOUND`. Cross-target locking matches `rename`.
